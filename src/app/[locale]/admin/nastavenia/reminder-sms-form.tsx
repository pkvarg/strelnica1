...
import { updateReminderSmsEnabled } from "./actions";

interface Props {
  current: boolean;
}

export function ReminderSmsForm({ current }: Props) {
  const t = useTranslations("settings");
  const tCommon = useTranslations("common");
  const [state, formAction, isPending] = useActionState(
    updateReminderSmsEnabled,
    null,
  );
      ...
      <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-zinc-800 bg-zinc-900/50 p-4 transition-colors hover:border-amber-600/50">
        ...
        <div>
          <p className="font-medium text-zinc-100">
            {current ? t("reminderSmsEnabled") : t("reminderSmsDisabled")}
          </p>
        </div>
      </label>
